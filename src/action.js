...
export const fetchJsonData = async function (url, data = {}, dataType = 'json', queryParams = {}) {
    ...
    if (!isFormData) {
        headers.append('Content-Type', dataType === 'json' ? 'application/json' : 'text/plain;charset=UTF-8');
    }
...
export class MyEncryption {
    ...
    increment(char, increment = 2) {
      ...
      const range = char === char.toUpperCase() ? 'ABCDEFGHIJKLMNOPQRSTUVWXYZ' : 'abcdefghijklmnopqrstuvwxyz';
