...
function PageBuilder() {
  ...
  const [selectedCategory, setSelectedCategory] = useState("My Pages");
  ...
  const [readymadeTemplates, setReadymadeTemplates] = useState([]);
  ...
  const handleCategoryChange = (category) => {
    ...
    if (category !== "My Pages") {
      setFilteredThemes(
        readymadeTemplates.filter((page) => page.category === category)
      );
    } else {
      setFilteredThemes(pageList);
    }
    setSearchTerm("");
  };
  ...
  useEffect(() => {
    if (templateListData) {
      setReadymadeTemplates(templateListData?.all_templates?.data);
      setCategoryOptions(() => [
        "My Pages",
        ...(Array.isArray(templateListData?.all_templates?.filter_options)
          ? templateListData.all_templates.filter_options
          : []),
      ]);
      setPageList(templateListData?.templates);
      setThemeList(templateListData.themes_list);
      const defaultTheme = templateListData?.themes_list?.find(
      ...
      if (searchTerm !== "") {
        ...
        setFilteredThemes(templateListData?.templates);
      }
    }
  }, [templateListData]);
  ...
  const handleQueryChange = (value) => {
    ...
    if (
      ...
      if (selectedCategory === "My Pages") {
        setFilteredThemes(pageList);
      } else {
        setFilteredThemes(
          readymadeTemplates.filter(
      ...
      if (selectedCategory !== "My Pages") {
        setFilteredThemes((prev) =>
          prev.filter((page) =>
            page?.name?.toLowerCase()?.includes(value.trim().toLowerCase())
          )
        );
      } else {
        setFilteredThemes((prev) =>
  ...
  const deleteTemplate = async () => {
    ...
    if (response?.status === true) {
      ...
      setFilteredThemes((prev) => prev.filter((obj) => obj.uid != deleteId));
      setSelectedPage(null);
      const formdata = new FormData();
      const result = await fetchData(getApiURL(`/page-builder`), formdata);
      if (result.status === true) {
        setPageList(result.templates);
      }
  ...
  const MovetoLive = async () => {
    setLiveLoading(true);
    const formdata = new FormData();
    formdata.append("theme_id", encryptor.encode(selectedPage?.theme_id, 42));
    formdata.append("type", selectedPage.type);
    formdata.append("graphql", selectedPage.uid);
    const response = await fetchData(getApiURL("/move-to-live"), formdata);
  ...
  const publishTemplate = async () => {
    ...
    if (
      ...
      setTemplateNameError("Page Internal Name is Required");
    ...
    if (!regex.test(internalName)) {
      setTemplateNameError("Invalid Input");
      return false;
    }
    ...
    const formdata = new FormData();
    formdata.append("asset", encryptor.encode(modalPageId, 42));
    formdata.append("theme id", encryptor.encode(selectedTheme?.id, 42));
      ...
      >
        {isListingApiCall ? (
          ...
          <Layout>
            <Layout.Section>
              <BlockStack gap={300}>
                {(pageList?.length > 0 || readymadeTemplates?.length > 0) && (
                  <TextField
                    fullWidth
                    value={searchTerm}
                ...
                {(pageList?.length > 0 || readymadeTemplates?.length > 0) && (
                  <>
                    ...
                    <InlineStack gap={200}>
                      {categoryOptions?.map((val, id) => {
                        return (
                          <Button
                            key={id}
                            pressed={selectedCategory == val}
            ...
            <Layout.Section>
                  ...
                  <BlockStack inlineAlign="center">
                    ...
                    {searchTerm && filteredThemes?.length === 0 ? (
                      <Text as="h2" tone="subdued" variant="bodyMd">
                        No page found matching your criteria. Please try a
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <div
                            className="Polaris-InlineGrid page-card"
                            style={{
                              gridGap: "10px",
                          ...
                          >
                            <div className="lazyContent" ref={addToRefs}>
                              {page.section_used_details ? (
                                page.type === "template-page" ? (
                                  <img
                                    className="pagebuild-card"
                                    src={page?.page_featured_image}
                                    alt={`Page image`}
                                    style={{
                                      width: "100%",
                                      height: "auto",
                                  ...
                                  <div className="lazyContent" ref={addToRefs}>
                                    {page.section_used_details?.map(
                                      (data, key) => (
                                        <img
                                          key={key}
                                          className="pagebuild-card"
                                          src={
                                            data?.page_builder_preview_img[0]
                                          }
                                          alt={`Page image ${key}`}
                                          style={{
                                            width: "100%",
                          ...
                          {selectedCategory === "My Pages" &&
                            (page?.status !== "unpublished" ? (
                              <Text>
                                Published to: <strong>{page.theme_name}</strong>
                              </Text>
                            ) : (
                              ...
                              <InlineStack gap={200} align="end">
                                ...
                                >
                                  <Button
                                    onClick={() =>
                                      window.open(
                                        `https://${SHOP}/apps/sectionhub/${page?.file_name
                                          ?.replace(".json", "")
                                          ?.replace(".liquid", "")}`
                                      )
                                    }
              ...
              {filteredThemes?.map((page, index) => (
                ...
                >
                  <div className="Polaris-Modal-Section">
                    <section
                      className="Polaris-Box page-img-modal"
                      style={{
                    ...
                    >
                      {page.section_used_details ? (
                        page.section_used_details?.map((data, id) => (
                          <img
                            key={id}
                            src={data.page_builder_preview_img[0]}
                            alt={page.name}
                          />
                        ))
                      ) : (
                        <img src={page.featured_img} alt={page.name} />
                      )}
                    </section>
                  </div>
                </Modal>
              ...
              <Modal
                ...
                primaryAction={{
                  content: "Proceed",
                  onAction: () => {
                    window.open(selectedPage.edit, "_blank"),
                      closeConfirmModal();
                  },
                }}
                secondaryActions={[
              ...
              >
                <Modal.Section>
                  <Text>
                    Are you sure you want to customize the{" "}
                    <strong> '{selectedPage?.name}' </strong> section in your
                    theme?
                  </Text>
                </Modal.Section>
              </Modal>
            ...
            <ConfirmationModal
              ...
              setIsOpen={setLiveConfirmModalOpen}
              text={
                "You are about to publish this page to the Live Theme. Please review and confirm before proceeding."
              }
              title={`Publish Page Confirmation`}
              buttonText={"Confirm"}
              buttonAction={MovetoLive}
              destructive={true}
              loading={liveLoading}
            />
            <ConfirmationModal
              ...
              text={
                "This action is irreversible. Deleting this page will permanently erase all its content. Would you like to continue?"
              }
            ...
            <Modal
              ...
              secondaryActions={{
                content:
                  publishSuccess?.status === true ? "Close" : "Add to theme",
                onAction:
                  publishSuccess?.status === true
                    ? () => {
                      ...
                      <BlockStack gap={500}>
                        <InlineStack gap={200}>
                          ...
                          <Text variant="bodyLg" as="h2">
                            Your Page has been successfully added and is now
